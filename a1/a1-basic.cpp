...
/*
	This is an simple implementation of  Frogger game in C++
	Rules: 
	  1.Press up button to go up
			  down button to go down
			  left button to go left 
			  right button to go right
			  'n' to go next game level
			  'q' to stop the game
	  2. When you go up to the top, you are done this level. 
	  	 	Now, you can only move left and right (not up and down),
	  		or, press 'n' to start next level.
	  3. When there's a collision between frog and the moving blocks, 
	  		frog will go back to initial place and start agian last level game.
	  4. After you are done all levels (4 in total), the game is literally finished. 
			i.e. you can do nothing but moving left and right for fun, or stop the game.
...
public:
	virtual void paint(XInfo& xinfo) {
		...
		XFillRectangle(xinfo.display, xinfo.window, xinfo.gc, x, y, w, h);
	}
...
public:	
	virtual void paint(XInfo& xinfo) {
		...
		//cout << "Now the player is in " << out << endl;
        XDrawImageString( xinfo.display, xinfo.window, xinfo.gc,
                          this->x, this->y, out.c_str(), 7); // string length is always 7
    }
...
int main( int argc, char *argv[] ) {
	...
	dList.push_back(new Block(0, 150, 100, 50));
	dList.push_back(new Block(425, 150, 100, 50));

	// draw second row of moving  blocks
	dList.push_back(new Block(0, 100, 20, 50));
	dList.push_back(new Block(210, 100, 20, 50));
	dList.push_back(new Block(425, 100, 20, 50));
	dList.push_back(new Block(630, 100, 20, 50));
	
	// draw third row of moving  blocks
	dList.push_back(new Block(0, 50, 50, 50));
	dList.push_back(new Block(280, 50, 50, 50));
	dList.push_back(new Block(560, 50, 50, 50));

	dList.push_back(new Block(-101, 150, 100, 50));
	dList.push_back(new Block(-101, 100, 20, 50));
	dList.push_back(new Block(-101, 50, 50, 50));

	Displayable *frog = new Block(initX, initY, FrogSize, FrogSize);
	repaint(dList, frog, xinfo);
	XFlush(xinfo.display);
